...
Changed gcode to remove preceeding 0 on commands to make gcode senders able to draw the image correctly.
"""
    ...
    def generate_gcode(self, curve, depth, laserPower, altfeed=None, altppm=None):
        ...
        for i in range(1,len(curve['data'])):
            ...
            if s[1] == 'move':
                ...
                gcode += "G0 " + self.make_args(si[0]) + " F%i " % self.options.Mfeed + "\n"
                lg = 'G0'
			...
			#G1 : Move with the laser turned on to a new point
                ...
                if not firstGCode: #Include the ppm values for the first G1 command in the set.
                    gcode += "G1 " + self.make_args(si[0]) + "S%.2f " % laserPower + "%s " % cutFeed + "%s" % ppmValue + "\n"
                    firstGCode = True
                else:
                    gcode += "G1 " + self.make_args(si[0]) + "\n"
                lg = 'G1'
